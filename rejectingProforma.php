<?php 
		...
		$query = "UPDATE invoice_order set state = '3' WHERE order_id='" . $_GET["invoice_id"]. "'";
		$result = $connect->query($query);
			if(!empty($result)) {
				...
				$query1 = "UPDATE booking set status = 8 WHERE id='" . $rowsd["bookingID"]. "'";
				$connect->query($query1);
			echo "<script> 
				var result = confirm('Thanks for your interest on Skylineautoservices.'); 
            if (result == true) { 
