...
$(document).ready(function() {
	...
	manageCategoriesTable = $('#manageCategoriesTable').DataTable({
		'ajax' : 'php_action/fetchServicesProviderGarage.php',
		'order': []
	...
	$('#addCategoriesModalBtn').unbind('click').bind('click', function() {
		...
		$("#submitCategoriesForm").unbind('submit').bind('submit', function() {
			...
			var categoriesName = $("#garageName").val();
			...
			if(servicesName == "") {
				...
				$("#serviceName").find('.text-danger').remove();
...
function editCategories(categoriesId = null ) {
	if(categoriesId) {
		...
		$.ajax({
			url: 'php_action/fetchSelectedServiceProviderGarage.php',
			type: 'post',
			...
			success:function(response) {
				...
				$("#editserviceName").val(response.service_ID);
				// set the categories name
				$("#editgarageName").val(response.provider_ID);
				// set the company TIN 
				// set the categories name
				...
				$("#editCategoriesForm").unbind('submit').bind('submit', function() {
					var categoriesName = $("#editserviceName").val(); 
					var wss_id = $("#editgarageName").val(); 
					var categoriesStatus = $("#editcategoryStatus").val();

					if(categoriesName == "") {
						$("#editserviceName").after('<p class="text-danger">Service name field is required</p>');
						$('#editserviceName').closest('.form-group').addClass('has-error');
					} else {
						// remov error text field
						$("#editserviceName").find('.text-danger').remove();
						// success out for form 
						$("#editserviceName").closest('.form-group').addClass('has-success');	  	
					}
					if(wss_id == "") {
						$("#editgarageName").after('<p class="text-danger">Garage field is required</p>');
						$('#editgarageName').closest('.form-group').addClass('has-error');
					} else {
						// remov error text field
						$("#editgarageName").find('.text-danger').remove();
						// success out for form 
						$("#editgarageName").closest('.form-group').addClass('has-success');	  	
					} 

					if(categoriesStatus == "") {
						$("#editcategoryStatus").after('<p class="text-danger">Service status field is required</p>');
	...
	$.ajax({
		...
		success:function(response) {			
			...
			$("#removeCategoriesBtn").unbind('click').bind('click', function() {
				...
				$.ajax({
					url: 'php_action/removeServiceProviderGarage.php',
					type: 'post',
					data: {categoriesId: categoriesId},
					dataType: 'json',
					success:function(response) {
...
function removeCategories1(categoriesId = null) {
	...
	$.ajax({
		url: 'php_action/fetchSelectedServiceProvider.php',
		type: 'post',
		data: {categoriesId: categoriesId},
		dataType: 'json',
		success:function(response) {			
			...
			$("#removeCategoriesBtn1").unbind('click').bind('click', function() {
				...
				$.ajax({
					url: 'php_action/activateServiceProviderGarage.php',
					type: 'post',
					data: {categoriesId: categoriesId},
					dataType: 'json',
					success:function(response) {
