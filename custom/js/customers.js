	...
	if(categoriesId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				// set the categories name
				$("#editstreet").val(response.street_number);
				// set the categories name
	...
	if(categoriesId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				// set the categories status
				$("#customerStatus1").val(response.status);
				// add the categories id 
				$(".editCategoriesFooter").after('<input type="hidden" name="editCategoriesId" id="editCategoriesId" value="'+response.id+'" />');
				...
				$.ajax({
					...
					success:function(response) {
						if(response.success == true) {
							...
							manageCustomersTable.ajax.reload(null, false);
							// udpate the messages
							$('.remove-messagesC').html('<div class="alert alert-success">'+
								'<button type="button" class="close" data-dismiss="alert">&times;</button>'+
				...
				$.ajax({
					...
					success:function(response) {
						if(response.success == true) {
							...
							manageCustomersTable.ajax.reload(null, false);
							// udpate the messages
							$('.remove-messagesU').html('<div class="alert alert-success">'+
								'<button type="button" class="close" data-dismiss="alert">&times;</button>'+
								'<strong><i class="fa fa-check"></i></strong> '+ response.messages +
								  '</div>');
