...
$(document).ready(function() {
	...
	$('#addCategoriesModalBtn').unbind('click').bind('click', function() {
		...
		$("#submitCategoriesForm").unbind('submit').bind('submit', function() {
			...
			if(categoriesName == "") {
				$("#categoriesName").after('<p class="text-danger">Brand12 Name field is required</p>');
			...
			if(tinNumber == "") {
				$("#tinNumber").after('<p class="text-danger">Tin number field is required</p>');
				$('#tinNumber').closest('.form-group').addClass('has-error');
			} else {
				// remov error text field
				$("#tinNumber").find('.text-danger').remove();
				// success out for form 
				$("#tinNumber").closest('.form-group').addClass('has-success');	  	
			}
			
			if(companyEmail == "") {
				$("#companyEmail").after('<p class="text-danger">Email field is required</p>');
				$('#companyEmail').closest('.form-group').addClass('has-error');
			} else {
				// remov error text field
				$("#companyEmail").find('.text-danger').remove();
				// success out for form 
				$("#companyEmail").closest('.form-group').addClass('has-success');	  	
			}
			
			if(companyPhone == "") {
				$("#companyPhone").after('<p class="text-danger">Phone number field is required</p>');
				$('#companyPhone').closest('.form-group').addClass('has-error');
			} else {
				// remov error text field
				$("#companyPhone").find('.text-danger').remove();
				// success out for form 
				$("#companyPhone").closest('.form-group').addClass('has-success');	  	
			}

			if(categoriesStatus == "") {
				$("#categoriesStatus").after('<p class="text-danger">Company Status field is required</p>');
			...
			if(categoriesName && tinNumber && companyEmail && companyPhone && categoriesStatus) {
				var form = $(this);
				// button loading
	...
	if(categoriesId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				// set the company TIN
				$("#editTin").val(response.TinNumber);
				// set the categories name
				$("#editEmail").val(response.po_email);
				// set the categories name
				$("#editPhone").val(response.po_phone);
				...
				$("#editCategoriesForm").unbind('submit').bind('submit', function() {
					...
					var companyEmail = $("#editEmail").val();
					var companyPhone = $("#editPhone").val();
					...
					if(companyEmail == "") {
						$("#editEmail").after('<p class="text-danger">Company Email field is required</p>');
					...
					if(companyPhone == "") {
						$("#editPhone").after('<p class="text-danger">Company Phone field is required</p>');
					...
					if(categoriesName && companyTin && companyEmail && companyPhone && categoriesStatus) {
						var form = $(this);
						// button loading
