...
function editCategories1(categoriesId = null ) {
	if(categoriesId) {
		...
		$.ajax({
			url: 'php_action/fetchSelectedBooking.php',
			type: 'post',
			...
			success:function(response) {
				...
				// set the company TIN
				$("#editGarage").val(response.garage); 
				// set the categories name
				$("#editMake").val(response.make);
				// set the categories name
				$("#editModel").val(response.model);
				// set the categories name
				$("#editPlate").val(response.plate_number);
				// set the categories name
				$("#editEngine").val(response.engine_number);
				// set the categories name
				$("#editColor").val(response.car_color);
				// set the categories name
				$("#editDescription").val(response.description);
				// set the categories name
				$("#editPickUpDate").val(response.pick_up_date);
				// set the categories name
				$("#editPickUptime").val(response.pick_up_time);
				// set the categories name
				$("#editDriver").val(response.email);
				// set the categories name
	...
	if(categoriesId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				// set the categories name
				$("#editphone").val(response.phone1); 
				// set the categories name
				$("#editemail1").val(response.email); 
				// set the categories status
				$("#customerStatus1").val(response.active);
				// add the categories id 
				$(".editCategoriesFooter").after('<input type="hidden" name="editCategoriesId" id="editCategoriesId" value="'+response.id+'" />');
				...
				$("#editCategoriesForm").unbind('submit').bind('submit', function() { 
					var categoriesStatus = $("#customerStatus1").val();
					...
					if(categoriesStatus == "") {
						$("#customerStatus1").after('<p class="text-danger">Customer status field is required</p>');
						$('#customerStatus1').closest('.form-group').addClass('has-error');
					} else {
						// remov error text field
						$("#customerStatus1").find('.text-danger').remove();
						// success out for form 
						$("#customerStatus1").closest('.form-group').addClass('has-success');	  	
					}
