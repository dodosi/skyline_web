...
$(document).ready(function() {
	...
	manageCustomersTable = $('#manageCustomersTable').DataTable({
		'scrollX': '200px',
		'scrollCollapse': true,
		'ajax' : 'php_action/fetchBookingPartner.php',
		'order': []
	...
	if(categoriesId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				// set the categories name
				$("#editzip").val(response.zipcode);
				// set the categories name
				
				$("#editservice").val(response.serviceName);
				// set the categories name
				$("#editmodel").val(response.model);
				// set the categories name
				$("#editmaked").val(response.make);
				// set the categories name
				$("#editplate").val(response.plate_number);
				// set the categories name
				$("#editcolor").val(response.car_color);
				// set the categories name
				$("#editpick").val(response.pick_up_date+' '+response.pick_up_time);
				// set the categories name
	...
	if(categoriesId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				// set the categories name
				$("#editservicess").val(response.serviceName); 
				// set the categories name
...
function editCategories2(categoriesId = null ) {
	if(categoriesId) {
		...
		// edit categories messages
		$("#comments-categories-messages").html("");
		// modal spinner
		$('.modal-loading').removeClass('div-hide');
		// modal result
		$('.edit-categories-result').addClass('div-hide');
		//modal footer
		$(".editCategoriesFooterD").addClass('div-hide');
		...
		$.ajax({
			...
			success:function(response) {
				...
				// submit of edit categories form
				$("#editCategoriesForm2").unbind('submit').bind('submit', function() { 
					var editemail1 = $("#editemail1").val();
					...
					if(comments == "") {
						$("#comments").after('<p class="text-danger">Comments are required</p>');
						$('#comments').closest('.form-group').addClass('has-error');
					} else {
						// remov error text field
						$("#comments").find('.text-danger').remove();
						// success out for form 
						$("#comments").closest('.form-group').addClass('has-success');	  	
					}
					...
					if(comments) {
						...
						//alert(editnames)
						//alert(editemail1)
						$("#editCategoriesBtn2").button('loading');
						...
						$.ajax({
							...
							success:function(response) {
								//alert("HERE")
								// button loading
								$("#editCategoriesBtn2").button('reset');
								...
								if(response.success == true) {
									...
									$('.form-group').removeClass('has-error').removeClass('has-success');
										...
										$('#comments-categories-messages').html('<div class="alert alert-success">'+
									'<button type="button" class="close" data-dismiss="alert">&times;</button>'+
									'<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> '+ response.messages +
									  '</div>');
